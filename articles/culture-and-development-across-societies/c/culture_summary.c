...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        fprintf(stderr, "Usage: %s data/cultural_development_panel.csv\n", argv[0]);
        return 1;
    }
    ...
    while (token != NULL && col < MAX_COLUMNS) {
        ...
        if (strcmp(token, "current_mismatch") == 0) mismatch_col = col;
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        while (field != NULL) {
            ...
            if (current_col == mismatch_col) mismatch = atof(field);
    ...
    printf("Mean current_mismatch: %.4f\n", mismatch_sum / count);
