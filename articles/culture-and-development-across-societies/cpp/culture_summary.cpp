...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cerr << "Usage: culture_summary data/cultural_development_panel.csv\n";
        return 1;
    }
    ...
    int mismatch_col = find_column(header, "current_mismatch");
    int support_col = find_column(header, "current_support");

    if (score_col < 0 || mismatch_col < 0 || support_col < 0) {
    ...
    while (std::getline(file, line)) {
        ...
        if (
            ...
            static_cast<int>(values.size()) <= mismatch_col ||
        ...
        mismatch_sum += std::stod(values[mismatch_col]);
    ...
    std::cout << "Mean current_mismatch: " << mismatch_sum / count << "\n";
