...
int main(int argc,char* argv[]){
  if(argc<2){ std::cerr<<"usage: regulation_summary data/regulation_development_panel.csv\n"; return 1; }
  std::ifstream file(argv[1]); std::string line; std::getline(file,line); auto h=split(line);
  std::vector<int> cols={idx(h,"regulation_score"),idx(h,"regulatory_support_context"),idx(h,"current_sleep"),idx(h,"acute_stress"),idx(h,"intervention_exposure")};
  std::vector<std::string> names={"regulation_score","regulatory_support_context","current_sleep","acute_stress","intervention_exposure"};
