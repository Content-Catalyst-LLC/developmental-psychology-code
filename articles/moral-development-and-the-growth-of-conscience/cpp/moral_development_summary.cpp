...
int main(int argc,char* argv[]){
  if(argc<2){ std::cerr<<"usage: moral_development_summary data/moral_development_panel.csv\n"; return 1; }
  std::ifstream file(argv[1]); std::string line; std::getline(file,line); auto h=split(line);
  std::vector<int> cols={idx(h,"conscience_score"),idx(h,"moral_action_score"),idx(h,"moral_support_context"),idx(h,"current_exclusion"),idx(h,"peer_pressure")};
  std::vector<std::string> names={"conscience_score","moral_action_score","moral_support_context","current_exclusion","peer_pressure"};
