...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        fprintf(stderr, "Usage: %s data/family_systems_panel.csv\n", argv[0]);
        return 1;
    }
    ...
    int development_col = -1, support_col = -1, parenting_col = -1, family_col = -1, stress_col = -1, col = 0;
    char *token = strtok(line, ",");

    while (token != NULL) {
        token[strcspn(token, "\r\n")] = 0;
        if (strcmp(token, "development_score") == 0) development_col = col;
        if (strcmp(token, "family_support_index") == 0) support_col = col;
        if (strcmp(token, "current_parenting") == 0) parenting_col = col;
    ...
    if (development_col < 0 || support_col < 0 || parenting_col < 0 || family_col < 0 || stress_col < 0) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        int current_col = 0;
        double development = 0.0, support = 0.0, parenting = 0.0, family = 0.0, stress = 0.0;
        char *field = strtok(line, ",");

        while (field != NULL) {
            if (current_col == development_col) development = atof(field);
            if (current_col == support_col) support = atof(field);
            if (current_col == parenting_col) parenting = atof(field);
    ...
    printf("Mean family_support_index: %.4f\n", support_sum / count);
    printf("Mean current_parenting: %.4f\n", parenting_sum / count);
